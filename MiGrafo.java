...
public class MiGrafo{
	...
	for(i = 0; i < vertices.length; i++)
		...
		for( int k = 0; k < vertices[i].ContarAdyacentes(); k++)
		    System.out.println(vertices[i].getAdyacente(k)); // Respresentacion String de Arista para este objeto
	    }
	...
	for(i = 0; i < vertices.length; i++)
	    {
	    	...
	    	for( int k = 0; k < vertices[i].ContarAdyacentes(); k++)
	    	    System.out.println(vertices[i].getAdyacente(k));
	    }	
    }
}

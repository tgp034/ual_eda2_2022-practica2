...
public class Grafo{
	...
	if(aristas.containsKey(arista.hashCode())) //arista ya está en el grafo?
	    return false;
	else if( v1.contieneAdyacente(arista) || v2.contieneAdyacente(arista)) //arista ya une a v1 o v2?
	    return false;

	aristas.put(arista.hashCode(), arista);
	v1.insertarAdyacente(arista);
	v2.insertarAdyacente(arista);
	return true;
    }
    ...
    {
	arista.getVertice1().eliminarAdyacente(arista);
	arista.getVertice2().eliminarAdyacente(arista);
    ...
    {
	return (this.vertices.get(vertice.getNombre()) != null);
    }
    ...
    {
	Vertice actual = this.vertices.get(vertice.getNombre());
	if(actual != null) //existía previamente?
	    {
		if(!sobreescribeVertice)
		    return false;

		while(actual.ContarAdyacentes() >= 0)
		    this.eliminarArista(actual.getAdyacente(0));
	...
	while(vertice.ContarAdyacentes() >= 0)
	    this.eliminarArista(vertice.getAdyacente(0));
